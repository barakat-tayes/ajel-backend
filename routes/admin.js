...
const ensureDriverModerationColumns = async () => {
  ...
  if (!existing.has("suspended_until")) {
    await pool.query("ALTER TABLE drivers ADD COLUMN suspended_until DATETIME NULL");
  }
  if (!existing.has("suspension_reason")) {
    await pool.query("ALTER TABLE drivers ADD COLUMN suspension_reason VARCHAR(20) NULL");
  }
...
const getAppSettings = async () => {
  ...
  const [rows] = await pool.query(
    ...
    ["contact_phone", "contact_whatsapp", "payment_master_card", "payment_zain_cash", "payment_asia_pay", "policy_text"]
  );
  const map = {};
  rows.forEach((r) => {
...
router.get("/settings", verifyAdmin, async (req, res) => {
  try {
    const [[row]] = await pool.query("SELECT username, commission_rate, per_order_fee FROM admins WHERE id=? LIMIT 1", [req.user.id]);
...
router.put("/settings", verifyAdmin, async (req, res) => {
  try {
    const hasPerOrderFee = Object.prototype.hasOwnProperty.call(req.body || {}, "per_order_fee");
    const perOrderFee = Number(req.body?.per_order_fee || 0);
    if (hasPerOrderFee) {
      await pool.query("UPDATE admins SET per_order_fee=? WHERE id=?", [perOrderFee, req.user.id]);
    }
    const keys = ["contact_phone", "contact_whatsapp", "payment_master_card", "payment_zain_cash", "payment_asia_pay", "policy_text"];
    ...
    const appSettings = await getAppSettings();
    res.json({ message: "updated", per_order_fee: perOrderFee, ...appSettings });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.put("/account", verifyAdmin, async (req, res) => {
  try {
    ...
    if (!username) return res.status(400).json({ error: "اسم مستخدم الأدمن مطلوب" });
    if (password && password.length < 3) {
      return res.status(400).json({ error: "كلمة المرور يجب أن تكون 3 أحرف على الأقل" });
    }
    if (password) {
      const hash = await bcrypt.hash(password, 10);
      await pool.query("UPDATE admins SET username=?, password=? WHERE id=?", [username, hash, req.user.id]);
    } else {
      await pool.query("UPDATE admins SET username=? WHERE id=?", [username, req.user.id]);
    }
...
router.get("/overview", verifyAdmin, async (req, res) => {
  try {
    const [[feeRow]] = await pool.query("SELECT per_order_fee FROM admins WHERE id=? LIMIT 1", [req.user.id]);
    const perOrderFee = Number(feeRow?.per_order_fee || 0);
    const [[restaurants]] = await pool.query("SELECT COUNT(*) count FROM restaurants WHERE status <> 'suspended'");
    const [[drivers]] = await pool.query("SELECT COUNT(*) count FROM drivers WHERE account_status = 'active'");
    ...
    const topProvince = [...provinceRows].sort((a, b) => (b.restaurants_count + b.drivers_count) - (a.restaurants_count + a.drivers_count))[0] || null;
    res.json({
...
router.get("/province/:name", verifyAdmin, async (req, res) => {
  try {
    ...
    const [[restaurants]] = await pool.query("SELECT COUNT(*) count FROM restaurants WHERE province=? AND status <> 'suspended'", [province]);
    const [[drivers]] = await pool.query("SELECT COUNT(*) count FROM drivers WHERE province=? AND account_status='active'", [province]);
    res.json({ province, restaurants: restaurants.count, drivers: drivers.count });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.put("/restaurants-ledger/:id/pay", verifyAdmin, async (req, res) => {
  try {
    ...
    if (remaining <= 0) {
      return res.status(400).json({ error: "لا يوجد مستحقات حالية على هذا التاجر" });
    }
    ...
    res.json({ message: "paid", paid_amount: appliedAmount, remaining_after_payment: newRemaining });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

router.put("/restaurants-ledger/:id/reset-due", verifyAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const range = String(req.body?.range || "month");
    let dateCond = "";
    if (range === "today") dateCond = "AND DATE(o.delivered_at) = CURDATE()";
    else if (range === "week") dateCond = "AND YEARWEEK(o.delivered_at, 1) = YEARWEEK(CURDATE(), 1)";
    else if (range === "year") dateCond = "AND YEAR(o.delivered_at) = YEAR(CURDATE())";
    else dateCond = "AND MONTH(o.delivered_at) = MONTH(CURDATE()) AND YEAR(o.delivered_at) = YEAR(CURDATE())";
...
router.put("/restaurants-ledger/:id/remind", verifyAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const [[rest]] = await pool.query("SELECT id, name FROM restaurants WHERE id=?", [id]);
    await pool.query("UPDATE restaurants SET last_payment_reminder_at=NOW() WHERE id=?", [id]);
    if (rest?.id) notifyUser("restaurant", rest.id, "admin_payment_reminder", { message: "تنبيه: يرجى تسديد المستحقات", restaurantName: rest.name });
...
router.put("/restaurants-ledger/:id/warn-suspension", verifyAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const [[rest]] = await pool.query("SELECT id, name FROM restaurants WHERE id=?", [id]);
    await pool.query("UPDATE restaurants SET suspension_warning_at=NOW() WHERE id=?", [id]);
    if (rest?.id) notifyUser("restaurant", rest.id, "admin_suspension_warning", { message: "تحذير: قد يتم غلق الحساب لعدم تسديد المستحقات", restaurantName: rest.name });
...
router.put("/restaurants-ledger/:id/toggle-warning", verifyAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const [[rest]] = await pool.query("SELECT id, name, suspension_warning_at FROM restaurants WHERE id=?", [id]);
    if (!rest?.id) return res.status(404).json({ error: "التاجر غير موجود" });
    const hasWarning = !!rest.suspension_warning_at;
    if (hasWarning) {
      await pool.query("UPDATE restaurants SET suspension_warning_at=NULL WHERE id=?", [id]);
      notifyUser("restaurant", id, "admin_clear_warnings", { message: "تم إلغاء التحذير من قبل الإدارة" });
      return res.json({ message: "warning_removed" });
    }
    await pool.query("UPDATE restaurants SET suspension_warning_at=NOW() WHERE id=?", [id]);
    notifyUser("restaurant", id, "admin_suspension_warning", { message: "تحذير: قد يتم غلق الحساب لعدم تسديد المستحقات", restaurantName: rest.name });
...
router.put("/restaurants-ledger/:id/suspend", verifyAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const [[rest]] = await pool.query("SELECT id, name FROM restaurants WHERE id=?", [id]);
    await pool.query("UPDATE restaurants SET status='suspended' WHERE id=?", [id]);
    if (rest?.id) notifyUser("restaurant", rest.id, "admin_account_suspended", { message: "تم غلق حسابك من قبل الإدارة", restaurantName: rest.name });
...
router.put("/restaurants-ledger/:id/toggle-suspend", verifyAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const [[rest]] = await pool.query("SELECT id, name, status FROM restaurants WHERE id=?", [id]);
    if (!rest?.id) return res.status(404).json({ error: "التاجر غير موجود" });
    if (rest.status === "suspended") {
      await pool.query("UPDATE restaurants SET status='active' WHERE id=?", [id]);
      notifyUser("restaurant", id, "admin_clear_warnings", { message: "تم فتح الحساب من قبل الإدارة" });
      return res.json({ message: "unsuspended" });
    }
    await pool.query("UPDATE restaurants SET status='suspended' WHERE id=?", [id]);
    notifyUser("restaurant", id, "admin_account_suspended", { message: "تم غلق حسابك من قبل الإدارة", restaurantName: rest.name });
...
router.put("/restaurants-ledger/:id/clear-flags", verifyAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    await pool.query(
...
router.get("/lookup-by-phone", verifyAdmin, async (req, res) => {
  try {
    ...
    if (!["restaurant", "driver"].includes(userType)) return res.status(400).json({ error: "نوع غير صالح" });
...
router.put("/drivers/:id/suspend-24h", verifyAdmin, async (req, res) => {
  try {
    ...
    const [[driver]] = await pool.query("SELECT id, name FROM drivers WHERE id=? LIMIT 1", [id]);
    ...
    notifyUser("driver", id, "admin_driver_suspended", {
      kind: "24h",
      message: "تم إيقاف حسابك لمدة 24 ساعة بسبب شكوى أو سوء استعمال. راجين منكم استخدام التطبيق بأفضل صورة من أجل استمرارية العمل للجميع.",
    });
...
router.put("/drivers/:id/suspend-72h", verifyAdmin, async (req, res) => {
  try {
    ...
    notifyUser("driver", id, "admin_driver_suspended", {
      kind: "72h",
      message: "تم إيقاف حسابك لمدة 72 ساعة بسبب شكوى أو سوء استعمال. راجين منكم استخدام التطبيق بأفضل صورة من أجل استمرارية العمل للجميع.",
    });
...
router.put("/drivers/:id/clear-suspend", verifyAdmin, async (req, res) => {
  try {
    ...
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ error: "معرف السائق غير صالح" });
    const [[driver]] = await pool.query("SELECT id FROM drivers WHERE id=? LIMIT 1", [id]);
    if (!driver?.id) return res.status(404).json({ error: "السائق غير موجود" });
...
router.delete("/drivers/:id", verifyAdmin, async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM order_rejected_drivers WHERE driver_id=?", [id]);
...
router.delete("/restaurants/:id", verifyAdmin, async (req, res) => {
  try {
    ...
    if (activeOrders.length) {
      return res.status(400).json({ error: "لا يمكن حذف حساب التاجر لوجود طلبات نشطة أو معلقة" });
    }

    await pool.query("DELETE FROM restaurant_settlements WHERE restaurant_id=?", [id]);
...
router.put("/approve/:type/:id", verifyAdmin, async (req, res) => {
  try {
    ...
    if (type === "restaurant") {
      ...
      await pool.query("UPDATE drivers SET account_status='active' WHERE id=?", [id]);
    } else {
...
router.put("/reject/:type/:id", verifyAdmin, async (req, res) => {
  try {
    ...
    if (type === "restaurant") {
      ...
      await pool.query("UPDATE drivers SET account_status='suspended' WHERE id=?", [id]);
    } else {
      return res.status(400).json({ error: "نوع غير صالح" });
    }
...
router.put("/temporary-password", verifyAdmin, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`SELECT id, name FROM ${table} WHERE username=? LIMIT 1`, [username]);
    if (!rows.length) {
      return res.status(404).json({ error: "لم يتم العثور على مستخدم بهذا الرقم" });
    }
    const hash = await bcrypt.hash(newPass, 10);
    await pool.query(`UPDATE ${table} SET password=? WHERE id=?`, [hash, rows[0].id]);
    res.json({ message: "تم تعيين كلمة مرور مؤقتة بنجاح", user: { id: rows[0].id, name: rows[0].name } });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
